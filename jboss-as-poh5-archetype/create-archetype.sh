...
sed -i 's;<name>jboss-javaee6-poh5-webapp-src</name>;<name>${name}</name>;' $ARCHETYPE_RESOURCES_DIR/pom.xml
...
if [ ! -z $1 ] && [ "$1" = "generate" ]; then
    ...
    mvn archetype:generate -B -DarchetypeCatalog=local \
        -DarchetypeArtifactId=jboss-javaee6-poh5-webapp -DarchetypeGroupId=org.jboss.spec.archetypes -DarchetypeVersion=7.0.0-SNAPSHOT \
        -DartifactId=example-project -DgroupId=com.acme -Dpackage=com.acme.example -Dversion=1.0.0-SNAPSHOT -Dname="Java EE 6 POH5 webapp project"
fi
